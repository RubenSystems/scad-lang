...
use crate::frontend::{mid_level_ir::mir_ast_types::{SSAValue, SSAExpression}, high_level_ir::ast_types::{FailureCopy, Type}};

use super::{tir_ast_expressions::{TIRExpression, TIRPhi}, tir_types::{TIRType, MonoType}, context::Context};
...
pub fn transform_mir_function_forward_decl_to_tir(
    ...
    match args.as_slice() {
        [tpe] => {
            let arg_type = if let m = tpe.to_tir_type() {
                m
            } else {
